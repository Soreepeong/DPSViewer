...
namespace FFXIVAPP.Memory
{
    public class MemoryHandler
    {
        ...
        {
            var file = Path.Combine(Directory.GetCurrentDirectory(), "structures-"+patchVersion+$"-{(processModel.IsWin64 ? "x64" : "x86")}.json");
            if (File.Exists(file))
            {
        ...
        {
            var file = Path.Combine(Directory.GetCurrentDirectory(), "enums-" + patchVersion + $"-{(processModel.IsWin64 ? "x64" : "x86")}.json");
            if (File.Exists(file))
            {
                using (var streamReader = new StreamReader(file))
                {
